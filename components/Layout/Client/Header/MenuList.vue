<template>
  <div :class="listContainerClasses">
    <ul class="pt-20 pl-12 flex flex-col gap-y-10 text-title">
      <li v-for="item in menuList">
        <NuxtLink :to="item.to" class="transition-colors duration-300 hover:text-first">
          {{ item.text }}
        </NuxtLink>
      </li>
      <li>
    ...
    <div class="absolute top-4 right-5 text-xl cursor-pointer text-title" @click="emit('close')">
